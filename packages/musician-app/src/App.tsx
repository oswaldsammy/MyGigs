...
const NavBar = () => {
  ...
  return (
    <header className="border-b bg-elev">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
        <Link to="/" className="text-xl font-bold text-brand-400">MyGigs · Musicians</Link>
        <nav className="flex items-center gap-6 text-sm">
          {user && <Link to="/" className="hover:text-brand-400">Dashboard</Link>}
          {user && <Link to="/gigs" className="hover:text-brand-400">Gigs</Link>}
          {user && <Link to="/messages" className="hover:text-brand-400">Messages</Link>}
          {user && <Link to="/reviews" className="hover:text-brand-400">Reviews</Link>}
          {user && <Link to="/profile" className="hover:text-brand-400">Profile</Link>}
          {user
            ? <button onClick={signOut} className="rounded-lg bg-elev px-3 py-1.5 hover:bg-line">Sign out</button>
            : <Link to="/auth/login" className="rounded-lg bg-brand-400 text-black px-3 py-1.5 text-white">Sign in</Link>}
        </nav>
      </div>
    </header>
  );
