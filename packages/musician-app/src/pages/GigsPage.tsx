    ...
    <div className="mx-auto max-w-4xl px-6 py-8">
      ...
      <div className="mb-4 flex gap-1 border-b overflow-x-auto">
        {(Object.keys(STATUS_FOR) as Tab[]).map((t) => (
          <button key={t} onClick={() => setTab(t)}
            className={`px-4 py-2 text-sm capitalize ${tab === t ? 'border-b-2 border-brand-400 text-brand-400' : 'text-gray-500 hover:text-white'}`}>
            {t}
          </button>
        ))}
      </div>
      ...
      {rows === null ? (
        ...
        <div className="space-y-3">
          {rows.map((b) => (
            <div key={b.id} className="rounded-xl border bg-elev p-4">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                <Link to={`/gigs/${b.id}`} className="rounded-lg border px-3 py-1.5 text-sm hover:bg-surface">
                  View details
                </Link>
                {tab === 'new' && (
                  <>
                    <button disabled={busy === b.id} onClick={() => quickAccept(b.id)}
                      className="rounded-lg bg-brand-400 text-black px-3 py-1.5 text-sm text-white disabled:opacity-50">
                      Accept
                    </button>
                    <button disabled={busy === b.id} onClick={() => quickDecline(b.id)}
                      className="rounded-lg border border-red-500/40 px-3 py-1.5 text-sm text-red-400 disabled:opacity-50">
                      Decline
                    </button>
                  </>
                )}
