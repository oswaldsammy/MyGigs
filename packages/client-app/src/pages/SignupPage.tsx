...
export const SignupPage = () => {
  ...
  const onSubmit = async (v: Form) => {
    setLoading(true);
    try {
      await signUpClient(v.email, v.password, v.fullName);
  ...
  return (
    <div className="mx-auto mt-16 max-w-sm rounded-xl bg-elev p-8 shadow">
      <h1 className="mb-6 text-2xl font-bold">Create a client account</h1>
      ...
      <p className="mt-4 text-center text-sm text-gray-400">
        Already have an account? <Link to="/auth/login" className="text-brand-400 hover:underline">Sign in</Link>
      </p>
    </div>
  );
};
